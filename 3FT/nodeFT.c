...
struct node {
   ...
   /* the type of node (if it is a file or directory) */
   NodeType type;
   /* pointer to the contents of a file */
   void* pvContents;
   /* the size of the contents in the file */
   size_t ulLength;
};
...
*/
int Node_new(Path_T oPPath, NodeType nodeType, Node_T oNParent, 
             Node_T *poNResult) {
   struct node *psNew;
